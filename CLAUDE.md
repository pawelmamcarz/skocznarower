...
- Tables: `bookings` (with `status` ∈ pending/confirmed/in_progress/done/cancelled, plus `final_price`, `reminder_sent_at`, `feedback_sent_at`, `gcal_event_id`, `repair_summary` (migration 0010, free-text "co zrobiono" quoted in the done SMS)), `blocked_slots` (PK `(date, time_slot)`; `time_slot='all'` blocks the whole day), `seasonal_reminders` (unique on `email`, plus `signed_up_at` and `sent_at` timestamps; the cron selects rows where `sent_at IS NULL` and stamps `sent_at` after sending), `google_reviews` (PK `review_id` from Google Places, upserted), `google_profile` (single row `id='profile'` with current rating + total review count), `outreach_contacts` (brand_name, channel A/B/C, status planned/sent/responded/closed, plus `contact_method`, optional `sent_at`/`response`/`notes`, and `created_at`/`updated_at`; seeded from `OUTREACH_PLAN.md` in migration 0006), `whatsapp_messages` (PK `wa_message_id`, plus `direction`/`wa_phone`/`body`/`created_at`; inbound WhatsApp log written by the webhook, migration 0009).
