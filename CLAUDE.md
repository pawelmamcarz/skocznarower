...
- **Vacation banner** (`#holiday-banner`): a static, date-gated banner ("Tryb wakacyjny do 12 lipca.") duplicated in **both** `index.html` and `umow.html`, each with its own inline script that only shows the banner while `Date.now() < 2026-07-12`. It auto-hides after that date with no Worker involvement. When the shop's vacation window changes, edit the visible copy **and** the hardcoded cutoff date in both files; remove both once it is permanently stale.
...
The Worker bundles the whole repo root as assets, so this file is what keeps junk out: large source media (`uploads/*.mp4`, `uploads/*.pdf`, `uploads/FastDL.to_*`), the Worker source (`src/`), migrations, dotfiles (`.wrangler/`, `.git/`, `.claude/`, `.playwright-mcp/`, `.DS_Store`), `node_modules/`, the `partners/` outreach docs, and the dev-only files (`CLAUDE.md`, `OUTREACH_PLAN.md`, `telnyx-voice-agent.md`, `.assetsignore`, `.gitignore`, `.dev.vars*`). When adding new top-level files or directories, decide whether they belong in `.assetsignore`.
