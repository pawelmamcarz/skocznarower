...
- **Vacation banner** (`#holiday-banner`): a static, date-gated banner ("Tryb wakacyjny do 12 lipca.") duplicated across `index.html`, `umow.html`, and the three SEO landing pages (`serwis-rowerow-pruszkow.html`, `serwis-rowerow-milanowek.html`, `bleeding-hamulcow-shimano.html`), each with its own inline script that only shows the banner while `Date.now() < 2026-07-12`. It auto-hides after that date with no Worker involvement. When the shop's vacation window changes, edit the visible copy **and** the hardcoded cutoff date in all of them; remove them once it is permanently stale.
...
- **Sources of truth that mirror the UI** live as constants at the top of `src/index.js`:
  ...
  - `PUBLIC_PHONE_DISPLAY` — the public-facing phone (`501 174 195`, Piotrek the serwisant's cell) used in SMS/email copy. Customers call this number directly; it replaced the Telnyx number `22 181 15 07`, whose voice agent never worked in production. It is **not** `OWNER_PHONE` (the owner-alert cell `600370810`) nor the WhatsApp `wa.me` number (also `600370810`). When the public number changes, update this constant **and** every page's visible phone, `tel:` links, JSON-LD `telephone`, `llms.txt`/`llms-full.txt`. Note: `600370810` is shown on the site **only** as a `wa.me` WhatsApp link, never as visible digits or a `tel:` link (decision: customers write on WhatsApp, not call that cell). In coexistence mode it is also the WhatsApp Cloud API sender, see the WhatsApp channel below.
...
- Booking lifecycle in `/admin` (`POST /admin/booking`, action-based, all via `adminUpdateBooking`): `confirm` (→ confirmed + calendar), `start` (→ `in_progress`, "przyjęto do serwisu", sends the customer an SMS via `repairAcceptedSms`), `done` (→ done, sends an SMS summary via `repairDoneSms`, and stores the optional `repair_summary` free-text typed in the row), `cancel`, `delete`, `price`. The two lifecycle SMS only fire on a real status transition (guarded by reading the row's prior status), so re-clicking does not re-send; they go out inline (awaited, not via cron). `repairDoneSms` quotes `repair_summary` if set else the service name, plus `final_price` if set, so set the price before clicking Zrobione. Status `in_progress` ("w naprawie") counts as active for `idx_bookings_active_slot` and shows under the **Nadchodzące** filter.
...
- Per-order detail + Mateusz/Piotr settlement (the row "Klient" name and an "Otwórz" button both link to `GET /admin/zlecenie?id=`, rendered by `adminBookingDetail`; saved via `POST /admin/zlecenie` action=finance → `adminSaveFinance`). The detail page edits `bike_model`, `repair_summary`, and the finance fields: `parts_cost` (actual spend), `parts_charged` (price to customer incl. markup), `labor_charge`, `amount_paid`, `payment_method` (cash/blik/transfer), `service_by` (piotr/mateusz), `parts_by` (mateusz/piotr/klient), `paid_to` (optional holder override). The settlement math lives in `computeSettlement`: **zysk = (parts_charged − parts_cost) + labor_charge**, split **75% Mateusz / 25% Piotr** (`SPLIT_MATEUSZ`, `Math.round` on Mateusz, Piotr = remainder); if `service_by='mateusz'` it is a solo job → **100% Mateusz, 0 Piotr**. `parts_cost` is a pass-through reimbursement to the `parts_by` buyer. The cash holder (`paymentHolder`) is `paid_to` if set else cash→Piotr, blik/transfer→Mateusz; net per person = collected − owed, positive = holds surplus and pays the other. Saving finance also sets `final_price = parts_charged + labor_charge` (the customer "wycena", keeps the done-SMS "Koszt" and dashboard revenue consistent) whenever either is provided. The **Rozliczenie** page (`GET /admin/rozliczenie` → `adminSettlement`, top-bar link) lists `done` jobs with per-order split and an aggregate "kto komu ile przekazać"; `POST /admin/rozliczenie` action=settle/unsettle toggles `settled_at` (settled jobs hide from the default view, visible under `?show=all`). Amounts are whole-złoty INTEGERs.
...
- Tables: `bookings` (with `status` ∈ pending/confirmed/in_progress/done/cancelled, plus `final_price`, `reminder_sent_at`, `feedback_sent_at`, `gcal_event_id`, `repair_summary` (migration 0010, free-text "co zrobiono" quoted in the done SMS), and the finance/settlement columns from migration 0011: `bike_model`, `parts_cost`, `parts_charged`, `labor_charge`, `amount_paid`, `payment_method`, `service_by`, `parts_by`, `paid_to`, `settled_at` — all whole-złoty INTEGERs except the TEXT enums; see the settlement note above), `blocked_slots` (PK `(date, time_slot)`; `time_slot='all'` blocks the whole day), `seasonal_reminders` (unique on `email`, plus `signed_up_at` and `sent_at` timestamps; the cron selects rows where `sent_at IS NULL` and stamps `sent_at` after sending), `google_reviews` (PK `review_id` from Google Places, upserted), `google_profile` (single row `id='profile'` with current rating + total review count), `outreach_contacts` (brand_name, channel A/B/C, status planned/sent/responded/closed, plus `contact_method`, optional `sent_at`/`response`/`notes`, and `created_at`/`updated_at`; seeded from `OUTREACH_PLAN.md` in migration 0006), `whatsapp_messages` (PK `wa_message_id`, plus `direction`/`wa_phone`/`body`/`created_at`; inbound WhatsApp log written by the webhook, migration 0009).
