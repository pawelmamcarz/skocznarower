...
- Tables: `bookings` (with `status` ∈ pending/confirmed/done/cancelled, plus `final_price`, `reminder_sent_at`, `feedback_sent_at`, `gcal_event_id`), `blocked_slots` (PK `(date, time_slot)`; `time_slot='all'` blocks the whole day), `seasonal_reminders` (unique on `email`), `google_reviews` (PK `review_id` from Google Places, upserted), `google_profile` (single row `id='profile'` with current rating + total review count), `outreach_contacts` (brand_name, channel A/B/C, status planned/sent/responded/closed, optional `sent_at`/`response`/`notes`; seeded from `OUTREACH_PLAN.md` in migration 0006).
